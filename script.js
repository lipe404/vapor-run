...
class Enemy {
  ...
  getRandomSpeed() {
    return (
      Math.random() *
        (GAME_CONFIG.ENEMY_MAX_SPEED - GAME_CONFIG.ENEMY_MIN_SPEED) +
  ...
  updateScale() {
    ...
    this.scale =
      GAME_CONFIG.ENEMY_INITIAL_SCALE +
      (GAME_CONFIG.ENEMY_FINAL_SCALE - GAME_CONFIG.ENEMY_INITIAL_SCALE) *
        progress;
  }
  ...
  getProgress() {
    return Math.min(
      1,
      (this.y - this.initialY) / (gameState.canvas.height - this.initialY)
    );
  }
  ...
  isOutOfBounds() {
    const scaledWidth = this.width * this.scale;
    return (
      this.y > gameState.canvas.height ||
      this.x < -scaledWidth ||
...
function spawnEnemies() {
  if (Math.random() < GAME_CONFIG.ENEMY_SPAWN_RATE) {
    const xPos =
      gameState.canvas.width / 2 - GAME_CONFIG.ENEMY_INITIAL_WIDTH / 2;
